...
func TestOutbox_ProcessOutBoxEvents_NoUnpublishedEvents_ReturnsNil(t *testing.T) {
	...
	mockRepo.On("GetUnPublished", ctx, 0, config.BatchSize, config.RetryThreshold).Return(events, nil).Maybe()
...
func TestOutbox_ProcessOutBoxEvents_FailToPublishEvent_ReturnsError(t *testing.T) {
	...
	mockRepo := mocks.NewMockRepository(t)
	mockRepo.On("GetUnPublished", ctx, 0, config.BatchSize, config.RetryThreshold).Return(events, nil)
	mockRepo.On("UpdatePublished", ctx, []types.ID{events[0].ID}, mock.Anything).Return(nil).Maybe()
...
func TestOutbox_ProcessOutBoxEvents_FailToUpdatePublished_ReturnsError(t *testing.T) {
	...
	mockRepo.On("UpdatePublished", ctx, []types.ID{events[0].ID}, mock.Anything).
...
func TestOutbox_ProcessOutBoxEvents_SuccessfullyPublished_ReturnsNil(t *testing.T) {
	...
	mockRepo := mocks.NewMockRepository(t)
	mockRepo.On("GetUnPublished", ctx, 0, config.BatchSize, config.RetryThreshold).Return(events, nil)
	mockRepo.On("UpdatePublished", ctx, []types.ID{events[0].ID, events[1].ID}, mock.Anything).Return(nil)
